 ...
 * ```
 */
class ShortcodeDirectives_Directive_Post_taxnomomy extends ShortcodeDirectives_Directive_Post_Base {
     ...
     * @return mixed|void
     */
    protected function _doAction( array $aAttributes, $aPost, $mData ) {
        ...
        $_aPosts    = $this->___getTargetPosts( $aPost, $_isTo );
         ...
         * @return array
         */
        private function ___getTargetPosts( $aPost, $isTo ) {

            // Case: to={post id} e.g. to=3455
            if ( is_numeric( $isTo ) ) {
                return array( get_post( $isTo, 'ARRAY_A' ) );
            }
            // Case: to=self
            if ( 'self' === $isTo ) {
                return array( $aPost );
        ...
        private function ___deleteTerms( array $aPosts, $sTaxonomySlug, array $aTerms ) {
            ...
            foreach( $aPosts as $_aPost ) {
                $_aoSetTerms = get_the_terms( $_aPost[ 'ID' ], $sTaxonomySlug );
                if ( is_wp_error( $_aoSetTerms ) ) {
                    continue;
                }
